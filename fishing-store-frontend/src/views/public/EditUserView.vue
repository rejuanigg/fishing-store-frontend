  ...
  <div class="min-h-[100dvh] bg-slate-50 px-5 pb-28 pt-8">
    <div class="mx-auto flex w-full max-w-screen-md flex-col gap-6">
      <section class="flex flex-col gap-2">
        ...
        <h1 class="text-3xl font-black tracking-tight text-blue-950">Gestioná tu cuenta</h1>
      ...
      <section class="rounded-[34px] border border-slate-100 bg-white p-5 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
        <form @submit.prevent="openWarningProfileModal" class="flex flex-col gap-6">
          <div class="flex flex-col gap-1">
            <h2 class="text-lg font-black text-blue-950">Información personal</h2>
            <p class="text-sm font-medium leading-6 text-slate-500">Actualizá los datos asociados a tu cuenta.</p>
          </div>

          <div class="flex flex-col gap-5">
            <div class="flex flex-col gap-2">
              <label class="text-sm font-black text-blue-900">Nombre</label>
              <input v-model="name" type="text" placeholder="Tu nombre" class="h-13 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-blue-950 outline-none transition placeholder:text-slate-400 focus:border-blue-500">
            </div>

            <div class="flex flex-col gap-2">
              <label class="text-sm font-black text-blue-900">Email</label>
              <input v-model="email" type="email" placeholder="tuemail@email.com" class="h-13 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-blue-950 outline-none transition placeholder:text-slate-400 focus:border-blue-500">
            </div>
          </div>

          <div class="flex flex-col gap-3 pt-2">
            <button type="submit" :disabled="!canUpdateProfile" class="h-13 rounded-2xl bg-blue-600 text-sm font-black text-white shadow-[0_14px_32px_rgba(5,150,105,0.22)] transition active:scale-[0.98] disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none disabled:active:scale-100">
      ...
      <section class="rounded-[34px] border border-slate-100 bg-white p-5 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
        <form @submit.prevent="openWarningPasswordModal" class="flex flex-col gap-6">
          <div class="flex flex-col gap-1">
            <h2 class="text-lg font-black text-blue-950">Seguridad</h2>
            <p class="text-sm font-medium leading-6 text-slate-500">Cambiá tu contraseña para proteger tu cuenta.</p>
          </div>

          <div class="flex flex-col gap-2">
            <label class="text-sm font-black text-blue-900">Contraseña actual</label>
            <input v-model="actualPassword" type="password" autocomplete="current-password" class="h-13 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-blue-950 outline-none transition focus:border-blue-500">
          </div>

          <div class="flex flex-col gap-2">
            <label class="text-sm font-black text-blue-900">Nueva contraseña</label>
            <input v-model="newPassword" type="password" autocomplete="new-password" class="h-13 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-blue-950 outline-none transition focus:border-blue-500">
          </div>

          <div class="flex flex-col gap-2">
            <label class="text-sm font-black text-blue-900">Confirmar nueva contraseña</label>
            <input v-model="newPasswordConfirm" type="password" autocomplete="new-password" class="h-13 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-blue-950 outline-none transition focus:border-blue-500">
          </div>

          <button type="submit" :disabled="!canChangePassword" class="h-13 rounded-2xl bg-blue-600 text-sm font-black text-white shadow-[0_14px_32px_rgba(5,150,105,0.22)] transition active:scale-[0.98] disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none disabled:active:scale-100">
