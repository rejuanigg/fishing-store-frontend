...
<template>
  <article v-for="order in orders" :key="order.id" class="overflow-hidden rounded-[30px] border border-blue-100/80 bg-white shadow-[0_14px_35px_rgba(15,23,42,0.06)] transition active:scale-[0.99]">

    <div class="p-5">
      <div class="flex items-start justify-between gap-4">

        <div class="min-w-0">
          <span class="block text-xs font-black uppercase tracking-[0.2em] text-blue-500">
            {{ auth.advancedAccess ? 'Orden' : 'Pedido' }}
          </span>

          <h3 class="mt-1 text-xl font-black tracking-tight text-blue-950">
            #{{ order.id }}
          </h3>
        ...
        <span class="flex h-9 shrink-0 items-center justify-center rounded-full px-4 text-xs font-black capitalize shadow-sm ring-1 ring-inset" :class="{
          ...
          'bg-blue-50 text-blue-700 ring-blue-200': order.status === 'completed',
      ...
      <div class="mt-5 grid grid-cols-2 gap-3">
        ...
        <div class="rounded-[24px] bg-slate-50 p-4">
          ...
          <p class="mt-2 text-xl font-black tracking-tight text-blue-700">
      ...
      <div class="mt-5 grid gap-3" :class="auth.advancedAccess ? 'grid-cols-[1fr_1.15fr]' : 'grid-cols-1'">
        ...
        <button @click="toDetailView(order.id)" class="flex h-12 items-center justify-center rounded-2xl bg-blue-500 px-4 text-sm font-black text-white shadow-lg shadow-blue-500/20 transition active:scale-[0.98]">
          {{ auth.advancedAccess ? 'Detalles' : 'Ver pedido' }}
        </button>

        <div v-if="auth.advancedAccess" class="relative">
          <select @change="updateStatus($event.target.value, order.id)" :value="order.status" class="h-12 w-full appearance-none rounded-2xl border border-slate-200 bg-white px-4 pr-10 text-sm font-black text-slate-600 outline-none transition focus:border-blue-400 active:scale-[0.98]">
    ...
    <div v-if="!auth.advancedAccess" class="border-t border-slate-100 bg-slate-50/70 px-5 py-4">
      <div class="flex items-center justify-between gap-3">
        <div>
          ...
          <span class="mt-1 block text-sm font-bold text-blue-950">
            {{ statusLabels[order.status] }}
          </span>
        </div>

        <button @click="toDetailView(order.id)" class="rounded-2xl bg-white px-4 py-2 text-sm font-black text-blue-700 shadow-sm ring-1 ring-slate-100 transition active:scale-[0.98]">
          Ver estado
        </button>
      </div>
    </div>
